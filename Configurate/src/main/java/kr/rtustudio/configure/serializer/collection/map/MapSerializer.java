package kr.rtustudio.configure.serializer.collection.map;
...
public class MapSerializer implements TypeSerializer.Annotated<Map<?, ?>> {
    ...
    public MapSerializer(final boolean clearInvalids) {
        ...
        this.fallback =
                requireNonNull(
                        TypeSerializerCollection.defaults().get(TYPE),
                        "Could not find default Map<?, ?> serializer");
    }

    @Override
    public Map<?, ?> deserialize(final AnnotatedType annotatedType, final ConfigurationNode node)
        ...
        if (node.isMap()) {
            if (!(annotatedType
                    instanceof final AnnotatedParameterizedType annotatedParameterizedType)) {
                throw new SerializationException(
                        type, "Raw types are not supported for collections");
            }
            ...
            for (final Map.Entry<Object, ? extends ConfigurationNode> ent :
                ...
                if (writeKeyBack) {
                    if (this.serialize(
                            ...
                            && !ent.getKey()
                                    .equals(
                                            requireNonNull(
                                                    keyNode.raw(), "Key must not be null!"))) {
    ...
    private @Nullable Object deserialize(
            final Type type,
            final TypeSerializer<?> serializer,
            final String mapPart,
    ...
    public void serialize(
        ...
        if (obj == null || obj.isEmpty()) {
            ...
            for (final Map.Entry<?, ?> ent : obj.entrySet()) {
                if (!this.serialize(
                        key.getType(), keySerializer, ent.getKey(), "key", keyNode, node.path())) {
                    continue;
                }
    ...
    @Override
    public @Nullable Map<?, ?> emptyValue(
