...
- **기능**: 단일 파일(`registerConfiguration`) 또는 폴더 전체(`registerConfigurations`) 여부를 기억하고, 각 설정에 대한 파일 경로(`Path`), 직렬화 방식(
  `Serializer`)을 함께 캐싱합니다.
...
비정적(Non-static) 내부 클래스로 구성된 설정 파트(`ConfigurationPart`를 상속한 이너 클래스)에 대해 자동으로 인스턴스화를 지원하며, `FieldProcessor`를 통해 설정값 병합/후처리
라이프사이클을 제공합니다.
