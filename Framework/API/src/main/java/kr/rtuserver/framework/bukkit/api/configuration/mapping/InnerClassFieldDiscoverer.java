...
public final class InnerClassFieldDiscoverer
    ...
    private final Map<
                    ...
                    List<Definition<?, ?, ? extends FieldProcessor.Factory<?, ?>>>>
            fieldProcessors;
    ...
    private static InnerClassFieldDiscoverer create(
            final Map<Class<?>, Object> overrides,
            final List<Definition<?, ?, ? extends FieldProcessor.Factory<?, ?>>> fieldProcessors) {
        final Map<
                        Class<? extends Annotation>,
                        List<Definition<?, ?, ? extends FieldProcessor.Factory<?, ?>>>>
                processors = new LinkedHashMap<>();
        for (final Definition<?, ?, ? extends FieldProcessor.Factory<?, ?>> definition :
                fieldProcessors) {
            processors
                    .computeIfAbsent(definition.annotation(), k -> new ArrayList<>())
                    .add(definition);
        }
        final InnerClassInstanceSupplier instanceSupplier =
                new InnerClassInstanceSupplier(overrides);
    ...
    public static FieldDiscoverer<?> config(
            final List<Definition<?, ?, ? extends FieldProcessor.Factory<?, ?>>> fieldProcessors) {
    ...
    @Override
    public @Nullable <V> InstanceFactory<Map<Field, DeserializedFieldInfo<?>>> discover(
            final AnnotatedType target,
            final FieldCollector<Map<Field, DeserializedFieldInfo<?>>, V> collector)
            throws SerializationException {
        final Class<?> clazz = erase(target.getType());
        if (ConfigurationPart.class.isAssignableFrom(clazz)) {
            // we don't care about the returned instance factory, we just need to call it to
            // collect
            // fields
            final Object dummyInstance =
                    this.delegate.<V>discover(
                            target,
                            (name, fieldType, container, deserializer, serializer) -> {
                                if (!erase(fieldType.getType())
                                        .equals(clazz.getEnclosingClass())) { // don't collect
                                    // synth fields for
                                    // inner classes
                                    ...
                                    for (final Annotation annotation : container.getAnnotations()) {
                                        final List<
                                                        Definition<
                                                                ...
                                                                ?,
                                                                ? extends
                                                                        FieldProcessor.Factory<
                                                                                ?, ?>>>
                                                definitions =
                                                        this.fieldProcessors.get(
                                                                annotation.annotationType());
                                        if (definitions != null) {
                                            for (final Definition<
                                                            ?,
                                                            ?,
                                                            ? extends FieldProcessor.Factory<?, ?>>
                                                    def : definitions) {
                                                if (GenericTypeReflector.isSuperType(
                                                        def.type().getType(),
                                                        GenericTypeReflector.box(
                                                                fieldType.getType()))) {
                                                    processor =
                                                            ((FieldProcessor.Factory) def.factory())
                                                                    .make(annotation, fieldType);
                                                    break processor;
                                                }
                                            }
                                        }
                                    ...
                                    collector.accept(
                                            ...
                                            (intermediate, newValue, implicitInitializer) -> {
                                                // we only create this map to collect the field, it
                                                // should only ever have 1
                                                // entry
                                                final SequencedMap<Field, Object> map =
                                                        new LinkedHashMap<>();
                                                deserializer.accept(
                                                        map, newValue, implicitInitializer);
                                                final Object deserializedValue;
                                                deserializedValue =
                                                        requireNonNullElseGet(
                                                                newValue,
                                                                () ->
                                                                        new ImplicitProvider(
                                                                                implicitInitializer));
                                                intermediate.put(
                                                        map.firstEntry().getKey(),
                                                        new DeserializedFieldInfo<>(
                                                                fieldType,
                                                                deserializedValue,
                                                                finalProcessor));
                                            },
