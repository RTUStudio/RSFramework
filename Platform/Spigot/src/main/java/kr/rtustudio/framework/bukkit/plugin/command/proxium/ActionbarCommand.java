...
public class ActionbarCommand extends RSCommand<RSFramework> {

    public ActionbarCommand(RSFramework plugin) {
        super(plugin, "actionbar", PermissionDefault.OP);
    }
    ...
    protected Result execute(CommandArgs data) {
        if (data.length() > 2) {
            ...
            target.sendActionBar(ComponentFormatter.mini(text));
