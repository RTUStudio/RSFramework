...
public abstract class RSPlugin extends JavaPlugin {
    ...
    public void onEnable() {
        if (MinecraftVersion.isSupport("1.19.3")) {
            plugin = this;
            adventure = BukkitAudiences.create(this);
        } else {
            Bukkit.getLogger().warning("Server version is unsupported version (< 1.19.3), Disabling this plugin...");
            Bukkit.getLogger().warning("서버 버전이 지원되지 않는 버전입니다 (< 1.19.3), 플러그인을 비활성화합니다...");
