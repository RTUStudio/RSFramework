...
/**
 * Loads and reloads per-plugin YAML configuration files. Supports plugin-info headers, version
 * management, and custom serializers.
 *
 * <p>플러그인별 YAML 설정 파일을 로드·리로드하는 구현체. 플러그인 정보를 포함한 헤더, 버전 관리, 커스텀 직렬화를 지원한다.
 *
...
public class PluginConfiguration<C extends ConfigurationPart> extends Configuration<C> {
    ...
    /**
     * Loads the configuration file and creates a new instance.
     *
     * <p>설정 파일을 로드하여 새 인스턴스를 생성한다.
     *
    ...
    /**
     * Reloads file contents into an existing instance.
     *
     * <p>기존 인스턴스에 파일 내용을 다시 로드한다.
     *
     * @param instance existing instance to reload
     */
