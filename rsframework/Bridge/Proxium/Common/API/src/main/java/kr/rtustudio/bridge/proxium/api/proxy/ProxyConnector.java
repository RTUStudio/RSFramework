...
import kr.rtustudio.bridge.proxium.api.protocol.handler.ProxyHandshakeHandler;
...
public class ProxyConnector {
    ...
    public ProxyConnector(
            @NonNull InetSocketAddress address, @NonNull String hostsFile, boolean tlsEnabled) {
        try {
            this.address = address;
            if (tlsEnabled) {
                trustManager =
                        new ProxiumTrustManager(
                                address.getHostName(), address.getPort(), hostsFile);
    ...
    public ProxyConnector(@NonNull InetSocketAddress address, boolean tlsEnabled) {
        this(address, ".", tlsEnabled);
    }

    public ProxyConnector(
            @NonNull String host, int port, @NonNull String hostsFile, boolean tlsEnabled) {
        this(new InetSocketAddress(host, port), hostsFile, tlsEnabled);
    }

    public ProxyConnector(@NonNull String host, int port, boolean tlsEnabled) {
        this(host, port, ".", tlsEnabled);
    }
        ...
        new Thread(
                        () -> {
                            try {
                                ...
                                if (f.isSuccess()) {
                                    ((ProxyHandshakeHandler) connection.getHandler())
    ...
    public boolean isConnected() {
        return !workerGroup.isShutdown() || !workerGroup.isShuttingDown() || connection.isOpen();
    }
